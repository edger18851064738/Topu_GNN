...
class VehicleState(Enum):
    ...
    PLANNING = "planning"
    WAITING = "waiting"
    CONFIRMED = "confirmed"  # 路径已确认，等待开始时间
    MOVING = "moving"
    BLOCKED = "blocked"
...
class Vehicle:
    ...
    def __init__(self, vehicle_id: int, start_node: str, target_node: str, 
        ...
        self.wait_until = 0.0
        self.path_start_time = 0.0  # 整条路径的开始时间
        self.retry_count = 0
        self.max_retries = 3
    ...
    def _execute_planning(self, current_time: float):
        ...
        if success:
            ...
            if self.use_gnn and self.path_times:
                self.path_start_time = max(self.path_times[0], current_time + 0.2)
            else:
                self.path_start_time = current_time + 0.2
            
            self.state = VehicleState.CONFIRMED
            print(f"车辆 {self.id}: 路径已确认，将在 {self.path_start_time:.1f}s 开始移动")
        else:
            # 预留失败，等待后重试
            self.road_network.cancel_reservations(self.id)
            self.wait_until = current_time + 0.5 + random.random() * 1.0
            self.state = VehicleState.WAITING
            print(f"车辆 {self.id}: 路径预留失败，等待重试")
    ...
    def _validate_and_reserve_path(self, current_time: float) -> bool:
        ...
        for i in range(len(self.path) - 1):
            ...
            start_time = max(self.path_times[i], current_time + 0.2)
            duration = self.path_times[i + 1] - self.path_times[i]
    ...
    def _arrive_at_node(self, current_time: float):
        ...
        if self.path_index + 1 < len(self.path):
            ...
            self.state = VehicleState.IDLE
            self.path_confirmed = False
            print(f"车辆 {self.id}: 到达目标 {self.current_node}")
    ...
    def _select_new_target(self):
        ...
        if available_nodes:
            ...
            print(f"车辆 {self.id}: 新目标 {self.target_node}")
...
class GNNMultiAgentSimulation:
    ...
    def _draw_vehicles(self):
        ...
        for vehicle in self.vehicles:
            ...
            # 绘制目标连线 - 只在非等待状态下显示
            if vehicle.state not in [VehicleState.WAITING, VehicleState.BLOCKED]:
                target_pos = self.road_network.node_positions[vehicle.target_node]
                line_alpha = 0.6 if vehicle.state == VehicleState.CONFIRMED else 0.4
                self.ax_main.plot([x, target_pos[0]], [y, target_pos[1]], 
                                color=vehicle.color, linestyle=':', 
    ...
    def _draw_confirmed_path(self, vehicle):
        ...
        self.ax_main.plot(xs, ys, color=vehicle.color, linewidth=4, 
                         alpha=0.3, linestyle='-', zorder=1)
        
        # 在路径节点上绘制小圆点
        for i, pos in enumerate(path_positions[1:], 1):  # 跳过起始点
            circle = Circle(pos, 0.08, color=vehicle.color, alpha=0.6, zorder=2)
            self.ax_main.add_patch(circle)
            self.ax_main.text(pos[0], pos[1], str(i), ha='center', va='center',
    ...
    def _draw_statistics(self):
        ...
        ║ 确认: {state_counts[VehicleState.CONFIRMED]:>13d} ║
...
def main():
    """主函数"""
    print("GNN多智能体道路协同演示 (延迟显示无冲突版本)")
    print("=" * 60)
    print("✨ 新特性: 延迟显示机制")
    print("- 车辆只有在确认路径完全可用时才开始移动动画")
    print("- 黄色边框表示路径已确认，等待开始时间")  
    print("- 绝对避免视觉冲突，确保动画的真实性")
    print("=" * 60)
    ...
    print("- P: 规划 (Planning)")  
    print("- W: 等待 (Waiting)")
    print("- C: 确认 (Confirmed) - 路径已确认，等待开始")
    print("- M: 移动 (Moving)")
    print("- B: 阻塞 (Blocked)")
    print("=" * 60)
